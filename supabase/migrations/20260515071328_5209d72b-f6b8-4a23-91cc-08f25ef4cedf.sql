ALTER TABLE public.data_source_status DROP CONSTRAINT IF EXISTS data_source_status_source_type_check;
ALTER TABLE public.data_source_status ADD CONSTRAINT data_source_status_source_type_check CHECK (source_type IN ('mentor_union','alumni_db','student_db','poc_db'));

ALTER TABLE public.data_source_sync_history DROP CONSTRAINT IF EXISTS data_source_sync_history_source_type_check;
ALTER TABLE public.data_source_sync_history ADD CONSTRAINT data_source_sync_history_source_type_check CHECK (source_type IN ('mentor_union','alumni_db','student_db','poc_db'));

-- Extend RPC to handle poc_db too, and backfill current student count
...
BEGIN
  ...
  ELSIF _source = 'poc_db' THEN
    SELECT count(*) INTO _count FROM public.poc_profiles;
  ELSE
    RETURN;
  END IF;
...
SELECT public.refresh_data_source_status('student_db');
SELECT public.refresh_data_source_status('poc_db');