...
BEGIN
  ...
  -- LMP creation is now mirrored by the app through an awaited sheets-lmp call.
  -- Do not enqueue or fire a second DB-triggered insert, otherwise concurrent
  -- app + trigger writes can race and create duplicate sheet rows.
  ...
  -- Immediate best-effort update for edits only. No background queue.
  BEGIN
